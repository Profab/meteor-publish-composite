...
Subscription.prototype.added = function (collectionName, doc) {
  this.refCounter.increment(collectionName, doc.id);

  if (this._hasDocChanged(collectionName, doc.id, doc)) {
    debugLog("Subscription.added", collectionName + ":" + doc.id);
    this.meteorSub.added(collectionName, doc.id, doc);
