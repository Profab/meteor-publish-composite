...
Publication.prototype.publish = function publish () {
  ...
  this.observeHandle = this.cursor.observe({
    added: function added (doc) {
      var alreadyPublished = self.publishedDocs.has(doc.id);

      if (alreadyPublished) {
        debugLog('Publication.observeHandle.added', collectionName + ':' + doc.id + ' already published');
        self.publishedDocs.unflagForRemoval(doc.id);
        self.subscription.changed(collectionName, doc.id, doc);
        self._republishChildrenOf(doc);
      } else {
        self.publishedDocs.add(collectionName, doc.id);
        self.subscription.added(collectionName, doc);
        self._publishChildrenOf(doc);
      }
    },
    changed: function changed (newDoc) {
      debugLog('Publication.observeHandle.changed', collectionName + ':' + newDoc.id);
      self._republishChildrenOf(newDoc);
      self.subscription.changed(collectionName, newDoc.id, newDoc);
    },
    removed: function removed (doc) {
      debugLog('Publication.observeHandle.removed', collectionName + ':' + doc.id);
      self._removeDoc(collectionName, doc.id);
    }
  });
};
...
Publication.prototype._publishChildrenOf = function _publishChildrenOf (doc) {
  _.each(this.childrenOptions, function createChildPublication(options) {
    ...
    this.publishedDocs.addChildPub(doc.id, pub);
    pub.publish();
  }, this);
};

Publication.prototype._republishChildrenOf = function _republishChildrenOf (doc) {
  this.publishedDocs.eachChildPub(doc.id, function (publication) {
...
PublishedDocumentList.prototype.add = function add (collectionName, docId) {
  if (! this.documents[docId]) {
    this.documents[docId] = new PublishedDocument(collectionName, docId);
  }
};
...
PublishedDocumentList.prototype.addChildPub = function addChildPub (docId, publication) {
  ...
  if (typeof doc === 'undefined') {
    throw new Error('Doc not found in list: ' + docId);
  }

  this.documents[docId].addChildPub(publication);
};
